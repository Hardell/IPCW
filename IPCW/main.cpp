	...
	for (int x = radius / 2; x < original.cols - radius / 2; ++x)
		...
		if (counter.at<uchar>(x, y, r) > thresholdValue)
		{
			...
			for (int r1 = min_radius; r1 < radius; ++r1)
				for (int x1 = x - radius / 16; x1 < x + radius / 16; ++x1)
					for (int y1 = y - radius / 16; y1 < y + radius / 16; ++y1)
			...
			{
				for (int r1 = min_radius; r1 < radius; ++r1)
					counter.at<uchar>(x, y, r1) = 0;
				continue;
			}
			rectangle(original, Point(x - radius / 16, y - radius / 16), Point(x + radius / 16, y + radius / 16), Scalar(255, 0, 0));
			putText(original, std::to_string(sumCount), Point(sumX, sumY),
			...
			for (int x1 = sumX - radius / 16; x1 < sumX + radius / 16; ++x1)
				for (int y1 = sumY - radius / 16; y1 < sumY + radius / 16; ++y1)
					for (int r1 = min_radius; r1 < radius; ++r1)
						if (counter.at<uchar>(x1, y1, r1) > thresholdValue)
						{
							sumX1 += x1;
